...
class UIManager:
    ...
    def load_all_assets(self):
        background_keys = ["background", "clue_board", "contestant", "LED_blue", "LED_red", "stage", "theme_background"]
        for key, filename in ASSET_FILES.items():
            ...
            if os.path.exists(path):
                ...
                if is_bg:
                    self.bgs[key] = pygame.transform.scale(img, target_size)
                else:
                    self.icons[key] = pygame.transform.scale(img, target_size)
            else:
    ...
    def draw_board_screen(self, game_manager):
        ...
        for p in game_manager.players:
            score_parts.append(f"{p.name}: ${p.score}")
    ...
    def draw_loading_screen(self, round_num):
        ...
        self.draw_text(f"Generating {rd_text} Board...", self.font_large, YELLOW, 640, 300)
        self.draw_text("Please wait ~15s (Generating AI Images takes longer!)", self.font_small, WHITE, 640, 350)