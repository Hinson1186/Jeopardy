import random
from players import Player, AI_Player  # Make sure this import is here!
    ...
    def populate_from_api(self, api_data: dict):
        ...
        for category_data in api_data.get('categories', []):
            ...
            for q_data in api_data.get('questions', []):
                if q_data.get('category') == cat_name:
                    clue = Clue(
                        category=cat_name,
...
class JeopardyGameManager:
    def __init__(self, llm_helper_instance):
        self.llm_helper = llm_helper_instance
        self.board = Board()
        self.players = [
            Player("Human"),
